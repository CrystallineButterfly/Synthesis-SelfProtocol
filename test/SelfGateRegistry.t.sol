...
contract SelfGateRegistryTest is Test {
    SelfGateRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_selfprotocol");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new SelfGateRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        SelfGateRegistry freshController = new SelfGateRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
